...
function M.qftf(info)
  ...
  local format_str = true and "%s %s:%s%s%s %s" or "%s│%s:%s%s%s│ %s"
...
function M.textobj_fold(motion)
  ...
  return (lev == 0 and "zj" or lev > levp and "j" or "") .. vim.v.count1 .. (motion == "i" and "[zjV]zk" or "[zV]z")
end
...
function M.goto_paragraph_firstline()
  ...
  while linenr >= 1 do
    local chunk = vim.api.nvim_buf_get_lines(0, math.max(0, linenr - chunk_size - 1), linenr - 1, false)
