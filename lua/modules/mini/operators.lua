...
local function dedent(lines)
  local indentAmounts = vim.tbl_map(function(line)
    return #(line:match "^%s*")
  end, lines)
  ...
  autocmd("FileType", {
    ...
    callback = function(ctx)
      ...
      local evalFunc = function(content)
        ...
        utils.notify("Eval", evaluatedOut)
      ...
      vim.b.minioperators_config = vim.b.minioperators_config and vim.tbl_deep_extend("force", conf, vim.b.minioperators_config) or conf
    end,
  })
end
...
local function luaEval(content)
  ...
  if parentDir:find "hammerspoon" then
    ...
    utils.notify("Eval", table.concat(output, "\n"))
  end
...
return {
  ...
  enabled = false,
  keys = {
    { "<leader>zs", desc = "󰅪 Substitute Operator" }, -- in visual mode, `s` surrounds
    { "<leader>zw", mode = { "n", "x" }, desc = "󰅪 Multiply Operator" },
    { "<leader>z#", mode = { "n", "x" }, desc = "󰅪 Evaluate Operator" },
    { "<leader>zsy", mode = { "n", "x" }, desc = "󰅪 Sort Operator" },
    { "<leader>zsx", mode = { "n", "x" }, desc = "󰅪 Exchange Operator" },
    { "<leader>zS", "s$", desc = "󰅪 Substitute to EoL", remap = true },
    { "<leader>zW", "w$", desc = "󰅪 Multiply to EoL", remap = true },
    { "<leader>z'", "#$", desc = "󰅪 Evaluate to EoL", remap = true },
  ...
  config = function(_, opts)
    ...
    require("mini.operators").make_mappings("replace", { textobject = "s", line = "ss", selection = "" })
  end,
}
