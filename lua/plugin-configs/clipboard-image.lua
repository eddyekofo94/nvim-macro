...
M.settings = {
  ...
  markdown = {
    ...
    img_handler = function(img)
      ...
      if y - 1 > 0 and not M.empty_line(y - 1)  then
        vim.cmd('normal! O')            -- add an empty line above
        vim.cmd('normal! j')
      end
      if y + 1 <= n_lines and not M.empty_line(y + 1) then
      ...
      while context_indent == -1
        ...
        offset = offset + 1
        if not M.empty_line(y - offset) then
          context_indent = vim.fn.indent(y - offset)
        end
        if not M.empty_line(y + offset)
...
M.clipboard_img.setup(M.settings)
