  ...
  if args.force_update == true or args.vifm_args ~= vim.g.vifm_pred_args then
    ...
    M.Vifm = Terminal:new {
      ...
      on_open = function(term)
        ...
        vim.keymap.set('t', '<CR>', [[<cmd>lua require('plugin-configs.toggleterm').vifm_set_open_method({cmd='e', type='file'})<CR><CR>]], keymap_opts)
        vim.keymap.set('t', 'l', [[<cmd>lua require('plugin-configs.toggleterm').vifm_set_open_method({cmd='e', type='file'})<CR>l]], keymap_opts)
        vim.keymap.set('t', 'i', [[<cmd>lua require('plugin-configs.toggleterm').vifm_set_open_method({cmd='e', type='file'})<CR>i]], keymap_opts)
        ...
        vim.keymap.set('t', '<leader>cd', [[<cmd>lua require('plugin-configs.toggleterm').vifm_set_open_method({cmd='cd', type='dir'})<CR>:qa<CR>]], keymap_opts)
        vim.keymap.set('t', '<leader>lcd', [[<cmd>lua require('plugin-configs.toggleterm').vifm_set_open_method({cmd='lcd', type='dir'})<CR>:qa<CR>]], keymap_opts)
        vim.keymap.set('t', '<leader>tcd', [[<cmd>lua require('plugin-configs.toggleterm').vifm_set_open_method({cmd='lcd', type='dir'})<CR>:qa<CR>]], keymap_opts)
      end,
      on_close = function()
        local file_name = Path:new(temp_file):read()
        local dir_name = Path:new(temp_dir):read()
        local cmd = M.open_method.cmd
        if cmd == nil or cmd == '' then cmd = 'e' end
        local target
        if M.open_method.type == 'dir' then target = dir_name
        elseif M.open_method.type == 'file' then target = file_name end
        if target == nil or target == '' then return end
        vim.schedule(function () vim.cmd(cmd .. ' ' .. target) end)
      end
    }
  end
  M.Vifm:toggle()
